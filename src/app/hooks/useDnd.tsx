...
interface DnDContextType {
  ...
  // The image URL of the item currently being dragged (for DragGhost).
  dragImg: string | null;
  setDragImg: Dispatch<SetStateAction<string | null>>;
}
...
export const useDnD = () => {
  ...
  useEffect(() => {
    ...
    const onDragEnd = (event: PointerEvent) => {
      ...
      const isDroppingOnSidebar =
        elementUnderPointer?.closest('[data-no-drop]');
      event.preventDefault();

      // Only allow dropping on the flow area, not on the sidebar (which lives inside .react-flow)
